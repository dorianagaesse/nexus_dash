...
- Validation: `npm run lint`, `npm run rls:check` (inventory matches
  ...
  including the updated ND-179 offboarding flow. Version policy moved
  0.73.0 -> 0.74.0 with the changelog entry. The edited migration was also
...
- Known flake (pre-existing, unrelated): `tests/scripts/version-policy.test.ts`
  spawns real git repositories and occasionally crosses the 5s default
  timeout under full coverage runs; it passes on rerun and the file is not
...
- Dropped `ProjectCalendarPanel`'s `canEdit` prop and the matching editor
  ...
  personal-calendar mutation affordances is no longer projected through the
...
- After explicit user approval to merge PR #453, refreshed the branch onto
  `origin/main` at `46688cf`. Application code merged cleanly; reconciled the
  two tracking conflicts by retaining TASK-356 as the active delivery brief
  and preserving TASK-348's release notes as `v0.49.0`.
- Advanced TASK-356 to `v0.50.0` because current `main` had already published
  `v0.49.0`, avoiding two independently merged product changes sharing one
  release identity.
- Post-reconciliation validation passed: release policy, lint, RLS inventory,
...
- Merge-forward follow-up: merged `origin/main` at `2db1386`, retaining the
  newer Epic archive/deep-link contract and advancing the fix release to
