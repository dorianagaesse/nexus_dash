-- ND-447: remove context-card stewardship. The stewardship model shipped
-- backend-only (ND-184) and was never surfaced; the unused "Resource" steward
-- columns, constraints, and indexes are dropped, along with the context-card
-- actor projection that existed only for the stewardship-era assignable-actor
-- list (the canonical app.list_project_actors projection remains). Context
-- cards keep their creator and last-editor provenance. Meeting-note
-- stewardship is untouched.

-- Replaced so the offboarding resolution no longer writes the dropped columns.
...
-- Context cards are projected from Prisma reads now; this SQL projection only
-- served the removed stewardship-era assignable-actor list.
DROP FUNCTION app.list_project_context_card_actors(TEXT);

DROP INDEX "Resource_stewardUserId_idx";
DROP INDEX "Resource_stewardCredentialId_idx";

ALTER TABLE "Resource"
  DROP CONSTRAINT "Resource_steward_actor_check";

ALTER TABLE "Resource"
  DROP CONSTRAINT "Resource_stewardUserId_fkey",
  DROP CONSTRAINT "Resource_stewardCredentialId_fkey";

ALTER TABLE "Resource"
  DROP COLUMN "stewardUserId",
  DROP COLUMN "stewardCredentialId",
  DROP COLUMN "stewardKind",
  DROP COLUMN "stewardDisplayNameSnapshot";
