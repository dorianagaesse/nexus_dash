...
export function KanbanBoard({
  ...
  const allKnownLabels = useMemo(() => {
    ...
    const collectLabel = (label: string) => {
    ...
    TASK_STATUSES.forEach((status) => {
      columns[status].forEach((task) => {
        task.labels.forEach(collectLabel);
      });
    });
  ...
  const searchableTaskRevision = useMemo(
    () => buildKanbanSearchRevision(allTasks),
    [allTasks]
  );
  const {
    normalizedQuery: normalizedTaskSearchQuery,
    searchTaskIds: effectiveSearchTaskIds,
    isLoading: isTaskSearchLoading,
    error: taskSearchError,
    retry: retryTaskSearch,
  } = useKanbanTaskSearch({
    projectId,
    query: taskSearchQuery,
    searchableTaskRevision,
  });
  const visibleColumns = useMemo(
    () => filterKanbanColumns(columns, effectiveSearchTaskIds, selectedLabels),
    [columns, effectiveSearchTaskIds, selectedLabels]
  );
  const visibleArchivedDoneTasks = useMemo(
    () =>
      archivedDoneTasks.filter((task) =>
        taskMatchesKanbanFilters(
          task,
          effectiveSearchTaskIds,
          selectedLabels
        )
      ),
    [archivedDoneTasks, effectiveSearchTaskIds, selectedLabels]
  );
  const hasActiveFilters =
    normalizedTaskSearchQuery.length > 0 || selectedLabels.size > 0;
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      const sourceTask = visibleColumns[sourceStatus][source.index];
      if (!sourceTask) {
        return;
      }

      const nextColumns = moveTaskUsingVisibleIndices({
        columns,
        visibleColumns,
        sourceStatus,
        ...
        transformMovedTask: (task, nextStatus) =>
          stampTaskActivity(
            {
              ...task,
              status: nextStatus,
      ...
      syncRelatedTaskSummary(sourceTask.id, {
  ...
  const shownTaskCount = useMemo(
    () =>
      TASK_STATUSES.reduce(
        (count, status) => count + visibleColumns[status].length,
        0
      ) + visibleArchivedDoneTasks.length,
    [visibleArchivedDoneTasks.length, visibleColumns]
  );
    ...
    >
      ...
      {isExpanded ? (
        <>
          <KanbanFilterToolbar
            query={taskSearchQuery}
            availableLabels={allKnownLabels}
            selectedLabels={selectedLabels}
            shownTaskCount={shownTaskCount}
            totalTaskCount={totalTaskCount}
            isSearchLoading={isTaskSearchLoading}
            searchError={taskSearchError}
            onQueryChange={setTaskSearchQuery}
            onToggleLabel={toggleLabelFilter}
            onClearLabels={() => setSelectedLabels(new Set())}
            onClearAll={() => {
              setTaskSearchQuery("");
              setSelectedLabels(new Set());
            }}
            onRetrySearch={retryTaskSearch}
          />
          <KanbanColumnsGrid
            canEdit={canEdit}
